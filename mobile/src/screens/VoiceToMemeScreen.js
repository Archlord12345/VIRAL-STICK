...
const VoiceToMemeScreen = ({ navigate }) => {
  ...
  const startRecording = () => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(micScale, { toValue: 1.15, duration: 600, useNativeDriver: true }),
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View>
            <Text style={[styles.screenTag, { color: theme.textMuted }]}>MODULE 2</Text>
        ...
        <GlassCard animate delay={100} style={styles.recorderCard}>
          ...
          <View style={styles.waveform}>
            {Array.from({ length: 24 }).map((_, i) => (
              <WaveBar key={i} index={i} isRecording={isRecording} theme={theme} />
            ))}
          </View>

          {/* Duration */}
          <Text style={[styles.duration, { color: isRecording ? theme.danger : theme.textMuted }]}>
            {isRecording ? `● REC ${formatDuration(duration)}` : formatDuration(duration)}
          </Text>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.micBtn,
                {
                  ...
                  ...createShadow(isRecording ? theme.danger : theme.primary, 16),
                },
              ]}
              activeOpacity={0.8}
            >
        ...
        {meme && (
          ...
          >
            <GlassCard style={styles.resultCard}>
              <Text style={[styles.resultTitle, { color: theme.secondaryLight }]}>
                💥 Mème Vocal Généré !
              </Text>
              <View style={[styles.memeBox, { backgroundColor: theme.backgroundSecondary, borderColor: theme.border }]}>
                <Text style={[styles.memeTop, { color: theme.textPrimary }]}>{meme.topText}</Text>
                <View style={styles.memeImage}>
                  <Text style={{ fontSize: 52 }}>🎙️</Text>
                </View>
                <Text style={[styles.memeBottom, { color: theme.textPrimary }]}>{meme.bottomText}</Text>
              </View>
              <AnimatedButton title="🔁 Recréer" onPress={generateMeme} variant="ghost" size="sm" style={{ marginTop: spacing.sm }} />
            </GlassCard>
          </Animated.View>
        )}
...
const styles = StyleSheet.create({
  ...
  recorderCard: { alignItems: 'center', paddingVertical: spacing.xl, marginBottom: spacing.md, gap: spacing.md },
  waveform: { flexDirection: 'row', alignItems: 'center', height: 60, gap: 4 },
  waveBar: { width: 4, height: 50, borderRadius: 4 },
  duration: { fontSize: typography.fontSize.lg, fontWeight: '700', letterSpacing: 2 },
  micBtn: {
    width: 90,
    height: 90,
    borderRadius: 45,
  ...
  transcriptLabel: { fontSize: typography.fontSize.xs, fontWeight: '700', letterSpacing: 2, marginBottom: spacing.sm },
  transcriptText: { fontSize: typography.fontSize.md, lineHeight: 24, fontStyle: 'italic' },
  resultCard: { marginBottom: spacing.md },
  resultTitle: { fontSize: typography.fontSize.xl, fontWeight: '800', textAlign: 'center', marginBottom: spacing.md },
  memeBox: { borderRadius: radius.md, borderWidth: 1, padding: spacing.md, alignItems: 'center' },
  memeTop: { fontSize: typography.fontSize.lg, fontWeight: '800', textTransform: 'uppercase', textAlign: 'center', marginBottom: spacing.sm },
  memeImage: { width: '100%', height: 140, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: radius.md },
  memeBottom: { fontSize: typography.fontSize.lg, fontWeight: '800', textTransform: 'uppercase', textAlign: 'center', marginTop: spacing.sm },
});
