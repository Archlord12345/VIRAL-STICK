...
const TOKEN = process.env.HUGGING_FACE_KEY || process.env.HF_TOKEN;
const CHAT_URL = "https://router.huggingface.co/v1/chat/completions";
const IMAGE_BASE = "https://router.huggingface.co/hf-inference/models";
...
async function main() {
  if (!TOKEN) {
    console.error(
      "Hugging Face token introuvable. Utilise HUGGING_FACE_KEY ou HF_TOKEN.",
    );
    process.exit(1);
  }

  console.log("=== TEST HUGGING FACE CHAT ===");
