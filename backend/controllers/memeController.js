...
const MemeController = {
  ...
  createFromVoice: async (req, res) => {
    try {
      ...
      let companionComment = "";
      try {
        companionComment = await AIService.chatWithCompanion('ubu',
  ...
  chat: async (req, res) => {
    try {
      ...
      const reply = await AIService.chatWithCompanion(companionId, message, history);
  ...
  getGreeting: async (req, res) => {
    try {
      ...
      const reply = await AIService.chatWithCompanion(companionId, "Salut ! Présente-toi brièvement selon ton rôle et souhaite-moi la bienvenue sur Viral Stick.");
      res.status(200).json({ reply });
    } catch (error) {
